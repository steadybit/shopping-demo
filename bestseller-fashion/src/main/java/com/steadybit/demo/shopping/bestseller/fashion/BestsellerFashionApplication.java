...
@SpringBootApplication public class BestsellerFashionApplication implements CommandLineRunner {
    ...
    @Override public void run(String... args) {
        ...
        List<Object[]> products = Stream.of(
                        new Object[] { randomUUID(), "Steadybit Hoodie", ProductCategory.FASHION.toString(), "hoodie", new BigDecimal("69.99") },
                        new Object[] { randomUUID(), "Steadybit Sun Glasses", ProductCategory.FASHION.toString(), "sunglasses", new BigDecimal("19.99") },
                        new Object[] { randomUUID(), "Steadybit Socks", ProductCategory.FASHION.toString(), "socks", new BigDecimal("9.99") })
        ...
        jdbcTemplate.query("SELECT id, name, category, imageId, price FROM products_fashion",
                (rs, rowNum) -> new Product(rs.getString("id"), rs.getString("name"), ProductCategory.valueOf(rs.getString("category")),
                        rs.getString("imageId"), rs.getBigDecimal("price"))).forEach(product -> log.info(product.toString()));
    }
